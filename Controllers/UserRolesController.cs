...
public class UserRolesController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<UserWithRolesDto>>> AssignRole(string userId,[FromBody] AssignRoleRequest request)
    {
        ...
        return Ok(ApiResponse<UserWithRolesDto>.SuccessResponse(userWithRoles, $"Role {request.Role} assigned to user {user.Id}"));
    }
    ...
    public async Task<ActionResult<ApiResponse<UserWithRolesDto>>> RemoveRole(string userId, string roleName)
    {
        ...
        if (!result.Succeeded)
        {
            return BadRequest(ApiResponse<IList<string>>.ErrorResponse($"Failed to remove role {roleName} from user {user.Id} User may not have this role."));
        }
        ...
        return Ok(ApiResponse<UserWithRolesDto>.SuccessResponse(userWithRoles, $"Role {roleName} removed from user {user.Id}"));
